...
with col1:
    st.subheader(f"![{away_team}]({away_logo}&w=20) {away_team} {away_wins}-{numGames - away_wins}")
    for game in away_team_l5:
        st.markdown(f"- {game}")

with col2:
    st.subheader(f"![{home_team}]({home_logo}&w=20) {home_team} {home_wins}-{numGames - home_wins}")
